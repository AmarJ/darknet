...
typedef struct imagesToScan {
    char *fileNameC;
    TAILQ_ENTRY(imagesToScan) nodes;
};
            ...
            if ( event->len ) {
               if ( event->mask & IN_CREATE ) {
                   if ( event->mask & IN_ISDIR ) {
                       ...
                       printf( "New file %s added to queue.\n", event->name );
		       e = NULL;
		       e = malloc(sizeof(struct imagesToScan));
		       e->fileNameC = strdup(event->name);
